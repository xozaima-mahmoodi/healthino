...
function buildReportDocument() {
  ...
  const refId = r.symptom_log_id
    ? `HLT-${r.symptom_log_id}`
    : `HLT-${Date.now().toString(36).toUpperCase()}`

  const urgency = triageUrgency.value
  const isEmergency = urgency?.key === 'emergency'
  ...
  const symptomChips = (a?.symptoms?.length ? a.symptoms : [])
    .map((s) => `<span class="chip">${escapeHtml(s)}</span>`)
    .join('')

  // Meta rows (severity / body area / duration) — only what we actually have.
  const metaRows = []
  if (a && Number.isFinite(a.severity)) {
    metaRows.push(`<div class="meta"><dt>${rt('severity')}</dt><dd>${escapeHtml(a.severity)}/10</dd></div>`)
  }
  if (a?.bodyArea) {
    metaRows.push(`<div class="meta"><dt>${rt('body_area')}</dt><dd>${escapeHtml(a.bodyArea)}</dd></div>`)
  }
  if (a && Number.isFinite(a.durationHours) && a.durationHours > 0) {
    metaRows.push(`<div class="meta"><dt>${rt('duration')}</dt><dd>${escapeHtml(a.durationHours)} ${rt('hours')}</dd></div>`)
  }

  const careItems = (firstAidItems.value || [])
    .map((item) => `<li>${escapeHtml(item)}</li>`)
    .join('')

  const doctorRows = (r.doctors || [])
    .map((d) => `
      <li>
  ...
  const specialtyBlock = r.specialty?.name
    ? `<section class="card">
         <h3>${rt('specialty')}</h3>
         <p class="specialty">${escapeHtml(r.specialty.name)}</p>
       </section>`
    : ''

  const redFlagBlock = r.red_flag
...
<body>
  <div class="page">
    ...
    <div class="content">
      ...
      <div class="classification"><span class="dot"></span>${escapeHtml(urgency?.label || '')}</div>
      ${redFlagBlock}
